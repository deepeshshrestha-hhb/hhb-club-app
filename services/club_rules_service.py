...
DEFAULT_SECTIONS = {
    ...
    "sitting_out": (
        ...
        "<li class=\"mb-1\"><strong>Coming back in:</strong> the group that just sat out swaps straight "
        "back in for whichever group's turn it is to sit out next. E.g. Players 17&ndash;20 sit out Game "
        "1; for Game 2, Player 20 takes over from Player 16, Player 19 from Player 15, Player 18 from "
        "Player 14, and Player 17 from Player 13.</li>"
        "<li class=\"mb-1\"><strong>Only here for an hour?</strong> You're still expected to sit out at "
        "least once during your time on court &mdash; the same fairness rule applies whether you're here "
        "for the full 2 hours or just 1.</li>"
    ...
    "pool_play": (
        ...
        "<p>For the second hour, players are split into two pools, each playing on its own pair of "
        "courts:</p>"
        "<ul class=\"ps-3 mb-2\">"
        "<li class=\"mb-1\">One pool plays <strong>Courts 1 &amp; 2</strong> (slightly better "
        "lighting), the other plays <strong>Courts 3 &amp; 4</strong>.</li>"
        "<li class=\"mb-0\"><strong>Which pool gets which courts alternates every Sunday</strong> "
        "<span class=\"text-muted small\">(from 27 Sep 2026)</span>, so both pools get equal time on "
        "Courts 1 &amp; 2 over the course of a season &mdash; check "
        "<a href=\"/sunday-pools\">/sunday-pools</a> for that week's specific Pool A / Pool B court "
        "assignment.</li>"
        "</ul>"
        "<p class=\"text-muted small mb-2\">Pool Play runs every Sunday during the 10&ndash;11am hour, "
        "independent of the Annual Players League season &mdash; the two pools and the alternating "
        "court assignment continue on their own schedule whether or not the League is currently in "
        "play.</p>"
        ...
        "<h6 class=\"fw-bold small text-uppercase text-muted mt-3 mb-2\">Sitting Out "
        "<span class=\"text-muted small text-lowercase fw-normal\">(updated 22 Sep 2026)</span></h6>"
        "<p>Each pool now signs in on its own table on the board &mdash; write your name under "
        "Pool A or Pool B (whichever you're in) in arrival order, same as the usual 9am sign-in. "
        "Each pool then rotates its own sit-outs from its own queue, independently of the other "
        "pool &mdash; the same first-in-sit-out-first, back-in-next-round rotation as the normal "
        "9am rules above, just run separately per pool.</p>"
        "<ul class=\"ps-3 mb-2\">"
        "<li class=\"mb-1\"><strong>Even turnout</strong> (e.g. 20 signed up, 10 per pool): every "
        "round, 2 sit out from Pool A's queue and 2 sit out from Pool B's queue, at the same "
        "time.</li>"
        "<li class=\"mb-0\"><strong>Uneven turnout</strong> (e.g. 19 signed up): the two pools "
        "alternate which one sits out that round &mdash; Pool A's queue sits out 2 one round, then "
        "Pool B's queue sits out 2 the next round, and so on.</li>"
        "</ul>"
        "<div class=\"table-responsive mt-2\">"
        "<table class=\"table table-sm table-bordered align-middle mb-1 hhb-rules-table\">"
        "<thead class=\"table-light\"><tr><th>Signed up</th><th>Pool A</th>"
        "<th>Pool B</th><th>Sitting out per round</th></tr></thead>"
        "<tbody>"
        "<tr><td>20 (even)</td><td>10</td><td>10</td>"
        "<td>2 from Pool A + 2 from Pool B, every round</td></tr>"
        "<tr><td>19 (uneven)</td><td>10</td><td>9</td>"
        "<td>2 from Pool A one round, then 2 from Pool B the next round &mdash; alternating</td></tr>"
        "</tbody></table>"
        "</div>"
