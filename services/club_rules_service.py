...
DEFAULT_SECTIONS = {
    ...
    "pool_play": (
        ...
        "<h6 class=\"fw-bold small text-uppercase text-muted mt-3 mb-2\">Sitting Out</h6>"
        "<p>Still one queue, in the order everyone signed the board at 9am &mdash; same as always. The "
        "only change for 10&ndash;11am: each round needs 2 sitting out from Pool A and 2 from Pool B, "
        "not just the next 4 names on the list. Work down the queue as normal, but skip anyone from a "
        "pool that's already got its 2 for that round, until both pools have their 2 &mdash; then mark "
        "X on the board as usual.</p>"
        "<p class=\"text-muted small mb-2\"><strong>Example:</strong> if the next four due to sit out "
        "are all Pool A, the first two sit out as normal &mdash; the next two are skipped (they keep "
        "playing) and the queue continues until two Pool B players are reached, who sit out instead.</p>"
        "<div class=\"table-responsive mt-2\">"
        "<table class=\"table table-sm table-bordered align-middle mb-1 hhb-rules-table\">"
